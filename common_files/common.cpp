...
bool is_valid_player_name(std::string &name) {
    for (char c: name) {
        if (!within_range(c, 33, 126)) {
            return false;
        }
    }
    return true;
...
uint32_t control_sum(const uint8_t *data, unsigned data_length) {
    ...
    for (unsigned int i = 0; i < data_length; i++) {
        index = (res ^ data[i]) & 0xFF;
        res = (res >> 8) ^ crc32_tab[index];
    }
    res ^= 0xFFFFFFFF;
    return res;
}
...
void rewrite(const uint8_t *source, uint8_t *destination, unsigned length, unsigned dest_start) {
    for (unsigned i = 0; i < length; ++i) {
        destination[dest_start + i] = source[i];
    }
}

void send_datagram(uint8_t *buffer, unsigned length, sockaddr_in6 &target, int socket_no) {
    sendto(socket_no, buffer, (size_t) length, 0,
           (struct sockaddr *) &target, (socklen_t) sizeof(target));
}
